    ...
    <div class="vip-club border_top_bottom vip-account">
        ...
        <div class="vip-club border_top_bottom">
            ...
            <ul class="border_bottom" style="clear:both;">
                <li style="float:left;"><img onclick=" location.href='/wap/goods/detail?id={{$good->id}}'" src="{{ \App\Models\Goods::getGoodsimages($good)[0] }}" width="80"/></li>
                <li style="float:left;margin-left:20px;margin-right:20px;font-size:18px;"><span ><?php echo mb_substr($good->name,0,10,'utf-8')."..."?></span><br><span style="color:red">￥{{ $good->price /100 }}</span></li>
            ...
            <ul style="clear:both;">
                <li  style="line-height:45px;text-align:right;margin-right:20px;">
                    ...
                    <input type="button" class="sub" value="确认付款" style="line-height:30px;border:solid 1px #84c1ff;background:#1980e0;color:#fff;width:80px;" onclick="pay();">
                    @else
                        <input type="button" class="sub" value="投诉" style="line-height:30px;border:solid 1px #ccc;background:#fff;color:#bbb;width:80px;">
...
@section('script')
    <script>
        {{--location.href='/wap/order/confirm?orders={{$orders->id}}--}}

        function pay(){
           if($("#div1").html().length==25){
               alert("请选择地址再付款！");
               location.href="/wap/user/address?a=1&orderid="+$("#orderid").val();
           }else{
               location.href='/wap/order/confirm?orders='+$("#orderid").val();
           }
        }
