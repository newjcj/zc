                    ...
                    <div class="plist">
                        <div class="list clearfloat fl alist" data-orderid="{{ $order->id }}" data-price="{{ \App\Models\Order::getOrderTotalPrice($order->id) }}" data-id="" style="height:1rem;font-size:-0.5rem;padding:0.1rem .2rem;margin-bottom:1%;margin-top:3%;">
                            <div class="xuan clearfloat fl" style="margin-top:0.3rem;">
                                <div class="radio">
                                    <label data-status="{{$order->cart_choice}}">
                                        <input type="checkbox" name="sex" value="">
                                    </label>
                                </div>
                            </div>
                            <div class="right clearfloat fl" style="width:72%">
                                <p class="tit over" style="font-size:0.4rem;line-height:0.9rem;"> {{ $order->shop->name }}&nbsp;&nbsp;&nbsp;下单时间:{{$order->created_at}} </p>
                                <div class="bottom clearfloat" style="margin-top:-0.8rem;margin-left:1.4rem;">
                                    <i data-id="{{ $order->id }}" class="iconfont icon-lajixiang fr"></i>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="list clearfloat fl blist" data-id="" style="height:2rem;">
                                ...
                                <a href="javascript:void(0)">
                                    ...
                                    <div class="right clearfloat fl" style="width:72%">
                                        ...
                                        <p class="price over" style="margin-top:-0.2rem;font-size:0.4rem;"> {{ $goods->price/100 }}元</p>
        ...
        <script type="text/javascript">
            ...
            $('.icon-lajixiang').click(function () { // 找到删除按钮，绑定点击事件
                ...
                layer.open({
                    ...
                    btn: ['确定', '取消'],
                    yes: function (index) {
                        $.post('/wap/order/del',{id:$(self).data('id'),_token: "{{csrf_token()}}"},function(){
            ...
            var tatol = function () {
                ...
                if (count > 0) {
                    $('.settlement span').html(count/100 + '元');
                    return true;
                }else{
