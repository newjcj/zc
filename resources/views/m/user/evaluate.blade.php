...
@section('content')
    ...
    <div class="vip-club border_top_bottom vip-account">
        <div class="vip-club-title border_bottom">
            <span style="width:45%;display:inline-block;text-align:center;color:red;" id="span1" onclick="move(1);">待评价</span>

            <span style="width:45%;display:inline-block;text-align:center;" id="span2" onclick="move(2);" >已评价</span>
        </div>
        {{--待评价--}}
       <div id="div1">
                        ...
                        @if($item->status==4)
                            @if(count($item->goodss))
                                @foreach($item->goodss as $good)
                                    ...
                                    <li style="float:left; margin-left: 20px; margin-top: 20px;"><img src="{{ \App\Models\Goods::getGoodsimages($good)[0] }}" width="120"/></li>
                                    <li style=" float: left; margin-left: 50px; margin-top: 20px; "><font color="#a52a2a">{{ $good->name}}</font></li>
                    ...
                    @if(count($orders)>0)
                        @foreach($orders as $item)
                            @if($item->status==4)
                                @if(count($item->goodss))
                                    @foreach($item->goodss as $good)
                                        @if($good->pivot->evaluate_status==2)
                                            <li style="float:left; margin-left: 20px; margin-top: 20px;"><img src="{{ \App\Models\Goods::getGoodsimages($good)[0] }}" width="120"/></li>
                                            <li style=" float: left; margin-left: 50px; margin-top: 20px; "><font color="#a52a2a">{{ $good->name}}</font></li>
                                            <li style="line-height:120px;text-align:right;margin-right:20px;">
