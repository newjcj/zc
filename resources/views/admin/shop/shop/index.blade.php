...
@section('content')
                ...
                @if(count($shop)>0)
                    @foreach($shop as $item)
                        <tr style="text-align: center">
                            ...
                            <td>
                                <select class="form-control input-small" id="regionid_{{$item->id}}" name="regionid" onchange="_change({{$item->id}});" >
                                    <option value="0" {{$item->certify==0 ? 'selected' : ''}}>审核中</option>
                                    <option value="1" {{$item->certify==1 ? 'selected' : ''}}>审核通过</option>
                                    <option value="2" {{$item->certify==2 ? 'selected' : ''}}>审核失败</option>
                                    <option value="3" {{$item->certify==3 ? 'selected' : ''}}>不允许提交</option>
                                </select>
                            </td>
    ...
    <script>
        ...
        function _change(id){
            ...
            Ajax('JSON').post('/admin/shop/shop/index', {
                id: id,
